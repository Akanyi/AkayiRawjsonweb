...
export class UI {
    constructor(appState, jsonConverter, modalManager, updateTagContent, editFeature) {
        ...
        this.currentSelectorTargetInputId = null; // 选择器编辑器的目标输入框ID
    ...
    updateSimulatorPreview() {
        ...
        if (!preview)
            return;
        const mockP = document.getElementById('mock-p')?.value || 'Steve';
        const mockR = document.getElementById('mock-r')?.value || 'Alex';
        ...
        try {
            ...
            parsed.rawtext.forEach((item) => {
                if (item.text) {
                    ...
                    const span = document.createElement('span');
                    if (item.selector.startsWith('@p'))
                        span.textContent = mockP;
                    else if (item.selector.startsWith('@r'))
                        span.textContent = mockR;
                    else if (item.selector.startsWith('@a'))
                        span.textContent = mockA;
                    else if (item.selector.startsWith('@s'))
                        span.textContent = mockS;
                    else if (item.selector.startsWith('@e'))
                        span.textContent = '[实体]';
                    else
    ...
    saveMockSettings() {
        const settings = {
            selectorNames: {
                '@p': document.getElementById('mock-p')?.value || 'Steve',
                '@r': document.getElementById('mock-r')?.value || 'Alex',
    ...
    // 为计分板名称显示选择器编辑器
    showSelectorEditorForScoreName(targetInputId) {
        ...
        const scoreNameInput = document.getElementById(targetInputId);
    ...
    // 公共方法，填充选择器输入框
    fillSelectorInput(selector) {
        if (this.currentSelectorTargetInputId) {
            const targetInput = document.getElementById(this.currentSelectorTargetInputId);
            if (targetInput) {
