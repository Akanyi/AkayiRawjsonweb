...
export class UI {
    ...
    private showCopyPreferencePrompt(jsonText: string): void {
        const content = `
        ...
        // 临时存储待复制的文本
        (window as any).__pendingCopyText = jsonText;
    }

    public setCopyPreferenceAndCopy(format: 'formatted' | 'compressed'): void {
        localStorage.setItem('copyJsonFormat', format);
        const jsonText = (window as any).__pendingCopyText;
        delete (window as any).__pendingCopyText;
    ...
    private doCopy(jsonText: string, compress: boolean): void {
    ...
    private getSettingsModalContent(): string {
    ...
    public saveSettings(): void {
        const formatSelect = document.getElementById('settings-copy-format') as HTMLSelectElement;
        const autoSaveCheckbox = document.getElementById('settings-auto-save') as HTMLInputElement;
        if (formatSelect) {
    ...
    private showAutoSavePrompt(): void {
        const content = `
            <div class="modal-content bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-sm">
    ...
    private getMockSettings(): { selectorNames: { [key: string]: string }, scoreMockValue: string, translateMockValue: string, conditionMockValue: string } {
    ...
    private getSimulatorModalContent(settings: { selectorNames: { [key: string]: string }, scoreMockValue: string, translateMockValue: string, conditionMockValue: string }): string {
        return `
    ...
    public updateSimulatorPreview(): void {
        ...
        const mockTranslate = (document.getElementById('mock-translate') as HTMLInputElement)?.value || '[翻译文本]';
        const mockCondition = (document.getElementById('mock-condition') as HTMLInputElement)?.value || '[条件输出]';
    ...
    public saveMockSettings(): void {
        const settings = {
            ...
            translateMockValue: (document.getElementById('mock-translate') as HTMLInputElement)?.value || '[翻译文本]',
            conditionMockValue: (document.getElementById('mock-condition') as HTMLInputElement)?.value || '[条件输出]'
        };
